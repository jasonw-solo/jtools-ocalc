...
function convertBer() {
    ...
    if (berInput.value && !hexInput.value) {
        try {
            ...
            const berValue = baseValue * Math.pow(10, expValue);
            
            if (berValue <= 0 || berValue >= 1) {
                throw new Error('BER值应在0到1之间');
            }
            
            // 转换为标准科学计数法：a × 10^b，其中1 ≤ a < 10
            const logValue = Math.log10(berValue);
            const b = Math.floor(logValue);
            let a = berValue / Math.pow(10, b);
            
            // 调整a和b，使得m为整数且在有效范围内
            // m = a × 10^k，需要m为整数且m ≤ 2047
            let m, s;
            for (let k = 0; k <= 3; k++) {
                m = Math.round(a * Math.pow(10, k));
                if (m > 0 && m <= 2047) {
                    s = b - k + 24;
                    if (s >= 0 && s <= 31) {
                        break;
        ...
        // 计算实际BER值
        const berValue = m * Math.pow(10, exp);
        
        // 转换为标准科学计数法，基数保留2位小数
        const logValue = Math.log10(berValue);
        const b = Math.floor(logValue);
        const a = berValue / Math.pow(10, b);
        
        // 格式化输出：基数保留2位小数
        const formattedBase = a.toFixed(2);
        const ber = `${formattedBase}E${b}`;
        berInput.value = ber;
